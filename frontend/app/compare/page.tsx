...
// Simple similarity check: normalize and compare first few words
...
export default function ComparePage() {
  ...
  // Multi-platform groups first, then single-platform
  ...
  const filteredMulti = multiPlatformGroups.filter(function (g) {
    return g.displayName.toLowerCase().includes(search.toLowerCase());
  });
  const filteredSingle = singlePlatformGroups.filter(function (g) {
    return g.displayName.toLowerCase().includes(search.toLowerCase());
  });
  ...
  return (
    <div className="page-container">
            ...
            {filteredMulti.map(function (group) {
              ...
              return (
                <div key={group.key} className="card">
                  <h3 className="text-[#f1f5f9] font-semibold mb-4">{group.displayName}</h3>
      ...
      {!loading && filteredSingle.length > 0 && (
        <div>
          <h2 className="text-lg font-bold text-[#f1f5f9] mb-4">📦 Tracked on One Platform</h2>
